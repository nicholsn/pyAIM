...
def list2Nested(fmaList):
    aalList = list()
    while fmaList:
        print "FMA Len",len(fmaList)
        tempList = list()
        for i in range(4):
            tempList.append(fmaList.pop())
            print len(fmaList),len(tempList), tempList
        print tempList
...
#fuctions designed for use in web service calls

aalArray = readAAL()

def aalID2aimByVoxel (aalID,fmriFile):
    '''for a given aalID (i.e. brain region) and fMRI file, generate and return an AIM xml file at the voxel by voxel level'''
    aalInt = int(aalID)
    aimTree = aimGen(voxelsByLabel(aalArray, aalInt, readFMRI(fmriFile)))
...
#return a list with the average zScore for a given brain region
def aalID2MeanLabel (aalID, fmriFile):
    aalInt = int(aalID)
    fmriArray = readFMRI(fmriFile)
    xyzLabels = voxelsByLabel(aalArray,aalInt,fmriArray) #list that stores mean aal and id
    voxelCount = 0
    zScoreSum = 0
    meanLabel = []
    for row in xyzLabels:
        voxelCount += 1
        zScoreSum += row[4]
    meanLabel.append(('','','',aalID,(zScoreSum/voxelCount)))
    return meanLabel

def aalID2aimMeanByLabel (aalID,fmriFile):
    '''for a given aalID (i.e. brain region) and fMRI file, generate and return an AIM xml file of mean zScore at the region level'''
    aimTree = aimGen(aalID2MeanLabel(aalID, fmriFile))
    aimTree.write('%s/AIM-%s.xml' % ('/Users/nolan/Documents/UW/GrantApps/Neuroimaging-Ontology/AIM-NIfTI/scratch', aalID)) #write out the aim file
    return

#return a list with the average zScore for all brain regions
def meanLabelsAll(fmriFile):
    meanAll = []
    for i in range (116):
        meanAll.append(aalID2MeanLabel((i+1),fmriFile))
    return meanAll

def aalID2aimThresh (aalid, thresh, fmriFile):
    aalInt = int(aalID)

#directory to write AIM file to
aimFile = '/Users/nolan/Documents/UW/GrantApps/Neuroimaging-Ontology/AIM-NIfTI/scratch'

b[len(sorted(a,key=lambda a:a[4]))-1]